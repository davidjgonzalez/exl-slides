...
import {
  getPreference, setPreference, getVisual, getAudioFilename,
  getNextStep, getPreviousStep,
  showAllSteps, showStep,
  getStepFromWindowLocation, updateWindowLocation,
} from './utils.js';
            ...
            <div class="sections">
              ...
              ${section.steps.map((step, stepIndex) => `
                  <div class="step" data-step="${step.id}" class="${step.active ? 'active' : ''}" 
                      ...
                      <h4 id="${step.id}" class="title">${section.number ? `${section.number}.` : ''}${step.number}. ${step.title}</h4>

                      <!-- Visuals -->
                      <div class="visual-wrapper">
                        <div class="visual ${step.visual.code ? 'code' : 'image'}">
                            <span class="step-counter">${stepIndex + 1}<span class="slash">/</span>${section.steps.length}</span>
                      ...
                      <div class="controls">
                          ...
                          <div class="step-name" data-step-name>
                              ...
                              <select class="section-title" data-section-select aria-label="Current section">
                                ${content.sections.map((sectionOption) => `<option 
                                  ${section.id === sectionOption.id ? 'selected' : ''}
                                  data-option-force-active="${section.id === sectionOption.id ? 'true' : 'false'}"
                                  value="${sectionOption.steps[0].id}">${sectionOption.title}</option>`).join('')}
                              </select>
                              ...
                              <select class="step-name-select" data-step-name-select aria-label="Current step">
                                ${section.steps.map((stepOption, stepOptionIndex) => `<option value="${stepOption.id}">${stepOptionIndex + 1}. ${stepOption.title}</option>`).join('')}
                              </select>
...
export default async function decorate(block) {
  ...
  await Promise.all([...block.querySelectorAll(':scope > div:not(:first-child)')].map(async (row) => {
    ...
    if (row.querySelector(':scope > div:last-child').innerHTML.trim() === '') {
      ...
      section = {
        ...
        description: [...row.querySelectorAll(':scope > div:first-child > :not(h3, h4)')].map((el) => el.outerHTML).join(''),
        steps: [],
      };
    } else {
      ...
      const stepId = section.id ? `${section.id}__${text.querySelector(':scope > h3, :scope > h4')?.id}` : text.querySelector(':scope h3, :scope > h4')?.id;
      ...
      section.steps.push({
        ...
        body: [...text.querySelectorAll(':scope > *')].filter((el) => !el.matches('h3, h4')).map((el) => el.outerHTML).join(' '),
